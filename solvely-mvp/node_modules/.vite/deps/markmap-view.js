...
var le = (e, t, n) => {
  ...
  let a = i, c = i.relX, s = o, l = o.relX, p = true;
  for (; a && s; ) {
...
var et = class _et {
  constructor(t, n) {
    ...
    }, this.svg = t.datum ? t : select_default(t), this.styleNode = this.svg.append("style"), this.zoom = zoom_default().filter((i) => this.options.scrollForPan && i.type === "wheel" ? i.ctrlKey && !i.button : (!i.ctrlKey || i.type === "wheel") && !i.button).on("zoom", this.handleZoom), this.setOptions(n), this.state = {
...
`) })
    ...
    this._checkImages(l), x.forEach((f) => {
      var y;
      (y = f.parentNode) == null || y.append(f.cloneNode(true));
    }), walkTree(t, (f, y, m) => {
      var b;
  ...
  renderData(t) {
    ...
    s(l);
    const p = l.descendants().reverse(), h = l.links(), u = linkHorizontal(), g = min(p, (r) => r.x - r.xSize / 2), x = max(p, (r) => r.x + r.xSize / 2), f = min(p, (r) => r.y), y = max(p, (r) => r.y + r.ySize - n);
   ...
   */
  async rescale(t) {
    const n = this.svg.node(), { width: i, height: o } = n.getBoundingClientRect(), a = i / 2, c = o / 2, s = transform(n), l = s.translate(
      (a - s.x) * (1 - t) / s.k,
      (c - s.y) * (1 - t) / s.k
    ).scale(t);

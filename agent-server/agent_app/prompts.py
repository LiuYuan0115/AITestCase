...
2.  **游戏场景洞察**：深刻理解游戏用户痛点，熟悉使用Unity、Unreal等游戏引擎，关注游戏体验闭环。
3.  **商业化设计**：熟悉各类付费墙（Paywall）设计、权益分层、用户生命周期价值（LTV）优化。
4.  **用户体验（UX）**：关注交互细节、情感化设计、以及 AI 响应速度/准确率对体验的影响。
5.  **技术边界认知**：了解 OCR、LLM、RAG 等技术的能力边界，能识别潜在的技术可行性风险。

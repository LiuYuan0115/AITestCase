...
.circle-index::before {
  @apply absolute h-[14px] w-[14px] rounded-full border-[4px] border-[#F0F2F4] bg-emph-disabled transition-colors duration-200 content-[''];
}

/* 内容区域样式 */
.content-wrapper {
  @apply relative min-w-0 flex-1 overflow-hidden pl-[8px];
}
...
/* 文本样式 */
.title {
  @apply font-sans text-base font-bold leading-[130%] text-emph-1;
}

.text {
  @apply font-sans text-base font-normal leading-[180%] text-emph-1;
}
</style>

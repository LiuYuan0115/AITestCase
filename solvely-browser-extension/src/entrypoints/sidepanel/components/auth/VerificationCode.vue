<template>
  <div v-show="isVisible"
    class="fixed inset-0 bg-[#FFFFFFCC] dark:bg-[#1B212DCC] backdrop-blur-[20px] transition-opacity duration-200 z-[1000] flex flex-col items-center justify-center">
    <!-- 关闭按钮 -->
    ...
    <!-- 主要内容 -->
    <div class="w-[330px] text-center relative animate-[scaleIn_0.2s_ease-out] flex flex-col items-center">
      <!-- 邮箱图标 -->
      <div class="mb-7">
...
// Emits
const emit = defineEmits(['close', 'verify-code', 'error'])
...
const globalValidCode = ref('') // 全局验证码状态
...
let codeValidTimer: NodeJS.Timeout | null = null // 验证码生效定时器
...
const hide = () => {
  isVisible.value = false
  resetState() // 只有authModal关闭时才完全重置
}
...
// 清理验证码生效定时器
const clearCodeValidTimer = () => {
  if (codeValidTimer) {
    clearTimeout(codeValidTimer)
    codeValidTimer = null
  }
  globalValidCode.value = ''
}

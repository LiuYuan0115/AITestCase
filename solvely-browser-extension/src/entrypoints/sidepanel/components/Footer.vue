<template>
  <footer class="p-4 pt-0 flex flex-col gap-[8px] w-full z-100">
    ...
    <div class="flex gap-[8px]">
      ...
      <UploadButton @click="handleUploadButtonClick" />
    ...
    <!-- 输入框 -->
    <div class="relative w-full border border-b_brand dark:border-b_brand_dk rounded-[16px] p-[11px] color-transtion">
      <!-- 文件上传组件 -->
      <FileStaging
        ref="fileStagingRef"
        @onSolve="handleSolve"
      ...
      <!-- 输入框 -->
      <div
        class="w-full pb-0 transition-[height] duration-200 ease-in-out"
      ...
      >
        <textarea
          ...
          v-model="inputValue"
          :class="[
            'w-full h-full outline-none text-[14px] leading-[1.4] font-normal text-t_1 dark:text-t_1_dk placeholder:text-t_3 dark:placeholder:text-t_3_dk bg-b_1 dark:bg-b_1_dk resize-none p-0 color-transition',
      ...
      <div class="w-full flex items-center justify-between pt-1">
        ...
        <div class="flex items-center gap-[8px]">
          <button
            v-if="canStop"
            class="w-[20px] h-[20px] flex items-center justify-center rounded-full bg-b_brand dark:bg-b_brand_dk hover:bg-b_brand_hov dark:hover:bg-b_brand_hov_dk text-s-interface-bg color-transition"
            :class="disabledStop ? '!bg-t_btn_dis dark:!bg-t_btn_dis_dk cursor-not-allowed' : ''"
    ...
    >
      ...
      /> -->
      <div class="w-[75px] h-[20px] flex items-center justify-center text-t_brand dark:text-t_brand_dk overflow-hidden">
        <SvgIcon
          name="sidepanel/upgrade"
          size="75"
        />
      </div>
    ...
    <PriceTicketBtn
      v-if="
        !isSubscribed && auth.isAuthenticated.value && auth.userFrom.value !== 'web' && auth.userFrom.value !== 'SEM'
      "
      class="absolute bottom-[13px] right-[16px]"
      @click="
        () => {
          trackEvent.track('Plugin_Coupon_Gift_Button_Click')
          $emit('showPriceTicket')
        }
      "
      :auth="auth"
    />
...
import ModelSelector from './solve/components/ModelSelector.vue'
import { STORAGE_KEY } from '@/config/storage'
import { PageScreenshotUploadManager } from '~/utils/PageScreenshotUploadManager'
import { uploadFileToS3 } from '~/utils/fileUpload'
import { useModelConfig } from './solve/composables/useModelConfig'
import PriceTicketBtn from './business/PriceTicketBtn.vue'

const { startStoreProcessByFileStackKey } = useRetrieveStore()
const { handleSummarizePage, handleQuizPage, trySniffCanvasUrlFromPage } = useCurrentWebsite()
const {
  stagingFileInfo,
  stagingStatus,
  stagingType,
...
const canSendButton = computed(() => {
  ...
  // 检查是否有引用文本且输入框有内容
  const hasQuoteText = quoteIsShow.value && quoteText.value && quoteText.value.trim()
  ...
  if (stagingStatus.value === 'loaded' && stagingType.value === 'image' && !hasQuoteText) {
    return true
  }
...
const isEmptyState = computed(() => {
  ...
  const isShowSubscriptionMessage = !!props.messageStore.pendingUserMessage?.value
...
const { handlePaste } = usePasteHandler({
  onFilePaste: async (file: File) => {
    if (!props.auth.isAuthenticated.value) {
      // 记录待处理动作，弹出登录框
      pendingAction.value = () => addFileOrUrl(file, false, undefined, undefined, 'Upload_Paste')
...
const handleSend = () => {
  ...
  if (stagingStatus.value === 'loaded' && stagingType.value === 'image' && !hasInputText) {
    handleImageSolve()
    clearStaging()
    return
  }
...
const handleCrop = contentScriptChecker.withContentScriptCheck(async () => {
  ...
  try {
    ...
    // 检查是否为扩展页面
    const isExtensionPage = tab.url?.startsWith('chrome-extension://') && tab.url?.includes('pdfView.html')
    if (isExtensionPage) {
      // 如果是扩展页面，发送消息给扩展页面
      try {
...
const handleChat = contentScriptChecker.withContentScriptCheck(async () => {
  ...
  try {
    ...
    uploadManager.setUploadFunctionInjector(() => {
      console.log('📤 [Footer handleChat] 注入上传函数（由第3个切片触发）')
    ...
    if (result.success && result.uploadManager) {
      ...
      console.log(`📸 [Footer handleChat] 截图完成，共 ${sliceCount} 个切片`)

      // ⭐ 4. 根据切片数决定是否上传
      if (sliceCount <= 2) {
        console.log('📦 [Footer handleChat] 小页面（<=2 切片），跳过上传')
        manager.markAsSkipped()
      } else {
        console.log(
          `📊 [Footer handleChat] 大页面（${sliceCount} 切片），已上传: ${manager.progress.value.uploaded}/${sliceCount}`
        )
      }
...
const handleQuotePrompt = async () => {
  ...
  const sliceValue = inputValue.value.length > 250 ? inputValue.value.slice(0, 250) : inputValue.value
...
const handlePdfSolve = () => {
  ...
  if (pdfPageCount && pdfPageCount > 20) {
    toastRef.value?.show(`File exceeds size limit. Only the first 20 pages will be processed.`)
  }
...
const handlePdfQuiz = () => {
  ...
  if (pdfPageCount && pdfPageCount > 50) {
    toastRef.value?.show(`File exceeds size limit. Only the first 50 pages will be processed.`)
  }
...
// 监听默认模型变化，保存到 storage
watch(defaultModel, async (newValue) => {
  if (newValue) {
    try {
      await browser.storage.local.set({
        [STORAGE_KEY.DEFAULT_SOLVE_MODEL]: newValue,
      })
      console.log('✅ Footer: 默认模型已更新', newValue)
      trackEvent.track('Plugin_Sidebar_DefaultModel_Changed', {
        modelId: newValue,
      })
    } catch (error) {
      console.error('❌ Footer: 保存默认模型失败', error)
    }
  }
})
...
onMounted(async () => {
  ...
  // 获取有效的默认模型（自动处理旧配置兼容）
  try {
    const { getEffectiveDefaultModel } = useModelConfig()
    defaultModel.value = await getEffectiveDefaultModel()
    console.log('✅ Footer: 当前有效的默认模型', defaultModel.value)
  } catch (error) {
    console.error('❌ Footer: 获取有效默认模型失败', error)
  }
  ...
  emitter.on('upload-error', (data: any) => {
    ...
    const isFileSizeError = error.includes('File size exceeds') || error.includes('file size is too large')
